...
internal sealed class Harness : IAsyncDisposable
{
    ...
    private readonly ISoemClient _soemClient = new SoemClient();
            ...
            try
            {
                switch (choice)
                {
                    ...
                    case "5":
                        ...
                        break;
                    case "6":
                        await SimulateSafeOpAsync().ConfigureAwait(false);
                        break;
                    case "7":
    ...
    private void PrintMenu()
    {
        ...
        Console.WriteLine(" 6) Safe-OP recovery simulation");
    ...
    private async Task InitializeAsync()
    {
        ...
        _service = new EthercatDriveService(_options, _loggerFactory.CreateLogger<EthercatDriveService>(), _soemClient);
    ...
    {
        _logger.LogError("Fault reported for slave {Slave}: {Error} (status={Status:X})", e.Slave, e.Error, (uint)e.Status);
        Console.WriteLine($"FAULT: slave {e.Slave} => {e.Error} status=0x{(uint)e.Status:X}");
    }
    ...
    private async Task ShowStatusAsync()
    {
        ...
        Console.WriteLine($"Cycle: last={snapshot.CycleDuration.TotalMilliseconds:F2} ms min={snapshot.MinCycleDuration.TotalMilliseconds:F2} ms max={snapshot.MaxCycleDuration.TotalMilliseconds:F2} ms");

        for (var i = 0; i < snapshot.DriveStatuses.Length; i++)
        {
            var status = snapshot.DriveStatuses[i];
            Console.WriteLine($"Slave {i + 1}: pos={snapshot.Positions[i]} status=0x{(uint)status:X} [{status}]");
        }
    }
    ...
    private async Task RunSoakTestAsync()
    {
        ...
        try
        {
            ...
            {
                var snapshot = service.GetStatus();
                var wkcHealthy = snapshot.Health.LastWkc >= snapshot.Health.GroupExpectedWkc;
                Console.WriteLine($"[{DateTimeOffset.UtcNow:HH:mm:ss}] WKC {snapshot.Health.LastWkc}/{snapshot.Health.GroupExpectedWkc} OP {snapshot.Health.SlavesOperational}/{count}");
                for (var i = 0; i < snapshot.DriveStatuses.Length; i++)
                {
                    Console.WriteLine($"  Slave {i + 1}: pos={snapshot.Positions[i]} status=0x{(uint)snapshot.DriveStatuses[i]:X}");
                }

                if (!wkcHealthy)
                {
                    Console.WriteLine("Work counter below expected threshold. Ending soak test.");
                    break;
                }
    ...
    private async Task SimulateSafeOpAsync()
    {
        var service = RequireService();
        var count = await service.GetSlaveCountAsync().ConfigureAwait(false);
        Console.WriteLine("Initiate a condition that will force the drives into SAFE-OP (e.g. engage E-stop). Press Enter when ready to monitor.");
        Console.ReadLine();

        Console.WriteLine("Monitoring for SAFE-OP transition...");
        while (true)
        {
            var snapshot = service.GetStatus();
            if (snapshot.Health.SlavesOperational < count || snapshot.Health.LastWkc < snapshot.Health.GroupExpectedWkc)
            {
                Console.WriteLine($"Detected SAFE-OP or degraded WKC (WKC {snapshot.Health.LastWkc}/{snapshot.Health.GroupExpectedWkc}, OP {snapshot.Health.SlavesOperational}/{count}).");
                break;
            }
        ...
        Console.WriteLine("Attempting automatic recovery...");
        for (var i = 1; i <= count; i++)
        {
            try
            {
                using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(5));
                await service.ResetAsync(i, cts.Token).ConfigureAwait(false);
                await service.EnableAsync(i, true, cts.Token).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Recovery command failed for slave {Slave}", i);
            }
        }

        Console.WriteLine("Recovery sequence issued. Observe status via option 4.");
    }
    ...
    private void ListNetworkAdapters()
    {
        ...
        {
            Console.WriteLine($"- {nic.Name} ({nic.Description}) [{nic.NetworkInterfaceType}] status={nic.OperationalStatus}");
        }
    }
}
